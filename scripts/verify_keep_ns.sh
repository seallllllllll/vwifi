...
if [ $final_ret -eq 0 ]; then
    ...
    mkdir -p logs
    sudo ip netns exec ns0 iw dev vw0 station dump | tee logs/station_dump_current.txt > station_dump_result.log    
    for num in {1..2}; do
...
if [ $final_ret -eq 0 ]; then
    echo "==== Test PASSED ===="
    echo "==== Keep vwifi module, hostapd, and namespaces for debugging ===="
    echo
    echo "Namespaces:"
    sudo ip netns list
    echo
    echo "Try:"
    echo "  sudo ip netns exec ns0 iw dev vw0 station dump"
    echo "  sudo ip netns exec ns1 iw dev vw1 set bitrates ht-mcs-2.4 7"
    echo "  sudo ip netns exec ns0 iw dev vw0 station dump"
    exit 0
fi
